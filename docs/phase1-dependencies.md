...
| Cursor | Bundled `Bibata-Modern-Amber` Xcursor and Hyprcursor payload | Hyprland environment, GTK 3/4, xsettingsd, gsettings, XDG default index, and Xcursor search path all select size 24 | The managed default lives only under `~/.local/share/icons/default/`; missing payload files stop installation before deployment. |
| Arabic fallback | `Noto Kufi Arabic` (`noto-fonts` on Arch; `media-fonts/noto` on Gentoo) | Language-specific user Fontconfig rule plus JetBrains-first Waybar/SwayNC/Rofi and Kitty Arabic-range fallback | Does not replace Latin or Nerd fonts; embedded website fonts and terminal bidi/cell behavior remain application-controlled. |
| Login manager | `sddm` on Arch; `x11-misc/sddm` on Gentoo plus `gui-libs/display-manager-init` for OpenRC | Standard SDDM theme, Hyprland Wayland session discovery, Bibata cursor fragment, next-boot enablement | Conflicting enabled managers require explicit approval; no autologin and no immediate start/restart. |
